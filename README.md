...
2. Update: On January 25, 2025, we fixed an issue in the code. Specifically, the previous version overestimated the I/O operations to a certain extent when determining the number of I/Os during cache reuse. When the fast memory capacity approaches its upper limit, the source code prematurely sets vol to the full-capacity state during the write-back estimation process, thereby evaluating the write-back volume under a worst-case assumption and consequently overestimating the operand size. In addition, during capacity reads when the fast memory is not fully occupied, the variable clear_data would more appropriately be set to 0. However, the old implementation also estimates the write-back volume according to the worst-case assumption by treating the write-back volume as equal to the read volume, which likewise leads to overestimation. Nevertheless, since both the overall analytical framework and the implementation itself remain applicable under such worst-case assumptions, and because the randomness and uncertainty of fast memory make such worst-case scenarios inherently important for evaluation, these issues do not affect the validity of the exploratory conclusions presented in this paper. This behavior can be regarded as a special case in which the original code evaluates performance under a worst-case assumption.

This issue has now been corrected. Please note that the search process described in this article is based on the original unfixed code, and we also provide the original data of the search process as a reference. For more rigorous academic discussion, we include both the original and updated versions of the code in the repository.

3. Due to resource constraints during the experiment, some parameters of the program were reduced based on the reference values ​​in the cited papers. For details, please refer to the specific parameter list in params.json under the corresponding case in TEST. These parameters can be adjusted when resources are sufficient.
